...
if args.model <= 50:
    ...
    micro_batch_size = 4
max_step = int(3600 * args.epoch / global_batch_size)
warmup_steps = 200
...
micro_batch_size = micro_batch_size if batch_size > micro_batch_size else batch_size
...
def setup(
    ...
    global out_dir
    hp_name = f'mdm-reverse-{args.model}M'
    out_dir = Path('workdir/reverse') / hp_name
...
def main(fabric, pretrain_path, resume):
    ...
    train_set = preprocess_reverse_train(tokenizer)
...
def train(fabric, state, train_dataloader, monitor, resume):
    ...
    for train_data in train_dataloader_:
        ...
        input_ids = train_data['data'] # [data + padding]
        length = train_data['length'] # data length
