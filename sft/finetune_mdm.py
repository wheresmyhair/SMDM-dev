...
def parse_args():
    ...
    parse.add_argument('--cfg', type=float, default=0., help='cfg drop ratio')
...
def setup(
    ...
    global out_dir
    hp_name = f'mdm-sharegpt-{args.model}M-cfg={args.cfg}'
...
def train(fabric, state, train_dataloader, monitor, resume):
    ...
    for train_data in train_dataloader_:
        ...
        if torch.rand(1) < args.cfg:
            noisy_input[prompt_index] = total_dim
