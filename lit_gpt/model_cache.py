...
class GPTCache(nn.Module):
    ...
    def forward(
        ...
        if use_kv_cache:
            current_pos = input_pos[-T :]
            cos = cos.index_select(0, current_pos)
            sin = sin.index_select(0, current_pos)
        else:
            cos = cos[:T]
            sin = sin[:T]
        ...
        if not use_kv_cache:
            ...
            for i, block in enumerate(self.transformer.h):
                x, self.kv_caches[i] = block(x, (cos, sin), max_seq_length, input_pos, self.kv_caches[i])
...
class Block(nn.Module):
    ...
    def forward(
        ...
        n_1 = self.norm_1(x)
        h, new_kv_cache = self.attn(n_1, rope, max_seq_length, input_pos, kv_cache)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            cache_k[:, input_pos[-1] - T + 1: input_pos[-1] + 1, :, :], cache_v[:, input_pos[-1] - T + 1: input_pos[-1] + 1, :, :] = k, v
            k = cache_k.index_select(1, input_pos)
            v = cache_v.index_select(1, input_pos)
